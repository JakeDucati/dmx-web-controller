...
export default function Timeline() {
    ...
    const [blocks, setBlocks] = useState<Block[]>([
        { id: "block-1", trackIndex: 0, startTime: 100, duration: 150, label: "Block 1" },
        { id: "block-2", trackIndex: 1, startTime: 200, duration: 100, label: "Block 2" },
    ]);
    ...
    const moveBlock = (blockId: string, newTrackIndex: number, newStartTime: number) => {
    ...
    return (
        <div>
            ...
            {/* Grid Toggle Button */}
            <Button onClick={toggleGrid} color="primary" className="mb-4">
                Grid
            </Button>
...
interface TrackProps {
    trackIndex: number;
    blocks: Block[];
    moveBlock: (blockId: string, newTrackIndex: number, newStartTime: number) => void;
...
function DraggableBlock({ block }: DraggableBlockProps) {
    ...
    return (
        ...
        >
            <div className="h-full w-2 left-0 bg-slate-600 rounded-s-md cursor-w-resize"></div>
            {block.label}
            <div className="h-full w-2 absolute right-0 bg-slate-600 rounded-e-md cursor-w-resize"></div>
        </div>
    );
}
